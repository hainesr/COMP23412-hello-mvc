...
* `Hello MVC JUnit test.launch` runs all the tests included for `Hello MVC` through the Eclipse JUnit runner.
* `Hello MVC package.launch` runs all tests through the Maven runner and then creates a runnable `jar` file, with all required dependencies, so that `Hello MVC` can be run from the command-line.
* `Hello MVC run.launch` runs `Hello MVC` within Eclipse.
* `Hello MVC test.launch` runs all the tests included for `Hello MVC` through the Maven runner.
